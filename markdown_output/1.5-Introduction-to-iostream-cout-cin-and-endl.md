...
> **Author:** Alex
> **Date:** Alex January 12, 2015, 4:53 pm PST February 5, 2025
...
Even though ‘\n’ is represented in source code as two symbols, it is treated by the compiler as a single linefeed (LF) character (with ASCII value 10), and thus is conventionally single quoted (unless embedded into existing double-quoted text). We discuss this more in lesson [4.11 -- Chars](https://www.learncpp.com/cpp-tutorial/chars/).
...
2. More importantly, it helps avoid inadvertent multicharacter literals. We cover multicharacter literals and some of the unexpected output they can cause in lesson [4.11 -- Chars](https://www.learncpp.com/cpp-tutorial/chars/).
...
- More importantly, it helps avoid inadvertent multicharacter literals. We cover multicharacter literals and some of the unexpected output they can cause in lesson [4.11 -- Chars](https://www.learncpp.com/cpp-tutorial/chars/).
...
- If your screen closes immediately after entering a number, please see lesson [0.8 -- A few common C++ problems](https://www.learncpp.com/cpp-tutorial/a-few-common-cpp-problems/) for a solution.
[0.8 -- A few common C++ problems](https://www.learncpp.com/cpp-tutorial/a-few-common-cpp-problems/)- If you are using CLion and “You entered” has a space before it, this is a bug in CLion. See [the CLion bug tracker](https://youtrack.jetbrains.com/issue/CPP-17579) for workarounds.
...
The C++ I/O library does not provide a way to accept keyboard input without the user having to press *enter*. If this is something you desire, you’ll have to use a third party library. For console applications, we’d recommend [pdcurses](https://pdcurses.org/), [FXTUI](https://github.com/ArthurSonzogni/FTXUI), [cpp-terminal](https://github.com/jupyter-xeus/cpp-terminal), or [notcurses](https://github.com/dankamongmen/notcurses). Many graphical user interface libraries have their own functions to do this kind of thing.
...
- If `std::cin` is not in a good state (e.g. the prior extraction failed and `std::cin` has not yet been cleared), no extraction is attempted, and the extraction process aborts immediately.
`std::cin`- Leading whitespace characters (spaces, tabs, and newlines at the front of the buffer) are discarded from the input buffer. This will discard an unextracted newline character remaining from a prior line of input.
...
We discuss how to detect and handle extraction failures, handle extraneous input, and clear `std::cin` in lesson [9.5 -- std::cin and handling invalid input](https://www.learncpp.com/cpp-tutorial/stdcin-and-handling-invalid-input/).
...
- If the user types `5a` and enter, `5a\n` will be added to the buffer. `5` will be extracted, converted to an integer, and assigned to variable `x`. `a\n` will be left in the input buffer for the next extraction.
`a\n`- If the user types ‘b’ and enter, `b\n` would be added to the buffer. Because `b` is not a valid integer, no characters can be extracted, so this is an extraction failure. Variable `x` would be set to `0`, and future extractions will fail until the input stream is cleared.
`0`- If `std::cin` is not in a good state due to a prior failed extraction, nothing happens here. The value of variable `x` is not altered.
...
- `std::cin` and `std::cout` always go on the left-hand side of the operator.
`std::cout`- `std::cout` is used to output a value (cout = character output).
`std::cout`- `std::cin` is used to get an input value (cin = character input).
`std::cin`- `<<` is used with `std::cout`, and shows the direction that data is moving. `std::cout << 4` moves the value `4` to the console.
`4`- `>>` is used with `std::cin`, and shows the direction that data is moving. `std::cin >> x` moves the value the user entered from the keyboard into variable `x`.
`>>`
