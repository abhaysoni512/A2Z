...
> **Author:** Alex
> **Date:** Alex June 27, 2007, 5:20 pm PDT October 13, 2024
...
- When defining an array, the length must be a compile-time constant. In the above program, `length` is non-const, so the definition `const int arr[length]` is not allowed.
`const int arr[length]`- Arrays are indexed from 0 to length-1. Therefore, `arr[length]` is an out-of-bounds index, and will lead to undefined behavior.
`arr[length]`- The array elements have type “const int”, and thus `arr[0]` cannot be modified by assignment.
