...
> **Author:** Alex
> **Date:** Alex November 26, 2024, 4:49 pm PST March 17, 2025
...
`if consteval`- Always test your constexpr functions in a constant context, as they may work when called in a non-constant context but fail in a constant context.
...
`constexpr`- `constexpr` is part of the interface of a function. Once a function is made constexpr, it can be called by other constexpr functions or used in contexts that require constant expressions. Removing the `constexpr` later will break such code.
`constexpr`- `constexpr` functions can be harder to debug since you can’t breakpoint or step through them in a debugger.
