...
> **Author:** nascardriver
> **Date:** nascardriver November 2, 2019, 7:17 am PDT November 26, 2024
...
To address the issue with `std::string` being expensive to initialize (or copy), C++17 introduced `std::string_view` (which lives in the <string\_view> header). `std::string_view` provides read-only access to an *existing* string (a C-style string, a `std::string`, or another `std::string_view`) without making a copy. **Read-only** means that we can access and use the value being viewed, but we can not modify it.
...
`std::string_view`- Convert an existing `std::string_view` to a `std::string` using `static_cast`
