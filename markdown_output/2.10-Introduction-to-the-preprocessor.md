...
> **Author:** Alex
> **Date:** Alex June 3, 2007, 12:57 pm PDT March 5, 2025
...
When the preprocessor encounters this directive, an association is made between the macro identifier and *substitution\_text*. All further occurrences of the macro identifier (outside of use in other preprocessor commands) are replaced by the *substitution\_text*.
...
Because PRINT\_JOE has been #defined, the line `std::cout << "Joe\n"` will be compiled. Because PRINT\_BOB has not been #defined, the line `std::cout << "Bob\n"` will be ignored.
...
This program prints “Bob”, because PRINT\_BOB was never *#define*d.
...
You can see a practical use of this feature in lesson [0.13 -- What language standard is my compiler using?](https://www.learncpp.com/cpp-tutorial/what-language-standard-is-my-compiler-using/).
...
Since we defined *PRINT\_JOE* to be nothing, how come the preprocessor didn’t replace *PRINT\_JOE* in *#ifdef PRINT\_JOE* with nothing and exclude the output statement from compilation?
...
Even though it looks like *#define MY\_NAME “Alex”* is defined inside function *foo*, the preprocessor doesn’t understand C++ concepts like functions. Therefore, this program behaves identically to one where *#define MY\_NAME “Alex”* was defined either before or immediately after function *foo*. To avoid confusion, you’ll generally want to #define identifiers outside of functions.
