...
> **Author:** Alex
> **Date:** Alex May 30, 2007, 9:54 pm PDT March 18, 2025
...
- Numbers (e.g. `5` or `-6.7`).
`-6.7`- Characters, which are placed between single-quotes (e.g. `'H'` or `'$'`). Only a single symbol may be used.
`'$'`- Text, which must be placed between double-quotes (e.g. `"Hello"` or `"H"`). Text can contain 0 or more characters.
...
Values that are placed directly into the source code are called **literals**. We cover literals in detail in lesson [5.2 -- Literals](https://www.learncpp.com/cpp-tutorial/literals/).
