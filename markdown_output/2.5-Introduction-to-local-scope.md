...
> **Author:** Alex
> **Date:** Alex February 8, 2015, 5:06 pm PST January 1, 2025
...
If the object is a class type object, prior to destruction, a special function called a destructor is invoked. In many cases, the destructor does nothing, in which case no cost is incurred. We introduce destructors in lesson [15.4 -- Introduction to destructors](https://www.learncpp.com/cpp-tutorial/introduction-to-destructors/).
...
- Execution starts at the top of `main`.
`main`- `main` variable `a` is created and given value `5`.
`5`- `main` variable `b` is created and given value `6`.
`6`- Function `add` is called with argument values `5` and `6`.
`6`- `add` parameters `x` and `y` are created and initialized with values `5` and `6` respectively.
`6`- The expression `x + y` is evaluated to produce the value `11`.
`11`- `add` copies the value `11` back to caller `main`.
`main`- `add` parameters `y` and `x` are destroyed.
`x`- `main` prints `11` to the console.
`11`- `main` returns `0` to the operating system.
`0`- `main` variables `b` and `a` are destroyed.
...
- The intended use of these variables isn’t apparent at the point of definition. You have to scan through the entire function to determine where and how each variable is used.
- The intended initialization value may not be available at the top of the function (e.g. we can’t initialize `sum` to its intended value because we don’t know the value of `x` and `y` yet).
`y`- There may be many lines between a variable’s initializer and its first use. If we don’t remember what value it was initialized with, we will have to scroll back to the top of the function, which is distracting.
...
- execution starts at the top of `main`
`doIt: x = 3 y = 4`- `doIt`‘s `y` and `x` are destroyed
`x`- `std::cout` prints `main: x = 1 y = 2`
`main: x = 1 y = 2`- `main` returns `0` to the operating system
`0`- `main`‘s `y` and `x` are destroyed
