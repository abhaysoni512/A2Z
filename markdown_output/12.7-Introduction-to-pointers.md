...
> **Author:** Alex
> **Date:** Alex July 10, 2007, 6:52 pm PDT February 7, 2025
...
If you’re rusty or not familiar with lvalue references, now would be a good time to review them. We cover lvalue references in lessons [12.3 -- Lvalue references](https://www.learncpp.com/cpp-tutorial/lvalue-references/), [12.4 -- Lvalue references to const](https://www.learncpp.com/cpp-tutorial/lvalue-references-to-const/), and [12.5 -- Pass by lvalue reference](https://www.learncpp.com/cpp-tutorial/pass-by-lvalue-reference/).
...
- When following a type name, & denotes an lvalue reference: `int& ref`.
`int& ref`- When used in a unary context in an expression, & is the address-of operator: `std::cout << &x`.
`std::cout << &x`- When used in a binary context in an expression, & is the Bitwise AND operator: `std::cout << x & y`.
