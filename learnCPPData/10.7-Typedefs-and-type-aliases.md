...
> **Author:** Alex
> **Date:** Alex June 19, 2007, 9:04 pm PDT February 11, 2024
...
The fixed-width integer types (such as `std::int16_t` and `std::uint32_t`) and the `size_t` type (both covered in lesson [4.6 -- Fixed-width integers and size\_t](https://www.learncpp.com/cpp-tutorial/fixed-width-integers-and-size-t/)) are actually just type aliases to various fundamental types.
