...
> **Author:** Alex
> **Date:** Alex January 29, 2008, 3:45 pm PST November 25, 2024
...
- It’s not clear what type `T` is supposed to be, as we’ve lost the documentation that `T` is intended to be an `Animal`.
`Animal`- This function does not enforce that `T` is an `Animal`. Rather, it will accept an object of any type that contains a `getName()` and `speak()` member function, whether that makes sense or not.
...
1. Our Animal/Cat/Dog example above doesn’t work like we want because a reference or pointer to an Animal can’t access the derived version of speak() needed to return the right value for the Cat or Dog. One way to work around this issue would be to make the data returned by the speak() function accessible as part of the Animal base class (much like the Animal’s name is accessible via member m\_name).

Update the Animal, Cat, and Dog classes in the lesson above by adding a new member to Animal named m\_speak. Initialize it appropriately. The following program should work properly:
