...
> **Author:** Alex
> **Date:** Alex November 24, 2015, 4:15 pm PST January 10, 2024
...
- The `std::vector` acquires new memory with capacity for the desired number of elements. These elements are value-initialized.
`std::vector`- The elements in the old memory are copied (or moved, if possible) into the new memory. The old memory is then returned to the system.
- The capacity and length of the `std::vector` are set to the new values.
...
The process of acquiring new memory at runtime is called dynamic memory allocation. We cover this in lesson [19.1 -- Dynamic memory allocation with new and delete](https://www.learncpp.com/cpp-tutorial/dynamic-memory-allocation-with-new-and-delete/).
