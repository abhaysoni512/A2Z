...
> **Author:** Alex
> **Date:** Alex January 24, 2008, 3:39 pm PST July 11, 2024
...
- `Label::setText()` and `Tooltip::setText()` have the same prototype. If we called `button.setText()`, the compiler would produce an ambiguous function call compilation error. In such cases, we must use the prefix to disambiguate which version we want.
`button.setText()`- In non-ambiguous cases, using the mixin name provides documentation as to which mixin the function call applies to, which helps make our code easier to understand.
