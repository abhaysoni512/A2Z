...
> **Author:** Alex
> **Date:** Alex September 19, 2018, 9:57 am PDT October 25, 2024
...
- Open the tasks.json file, find `"args"`, and then locate the line `"${file}"` within that section.
`"${file}"`- Above the `"${file}"` line, add a new line containing the following commands:
...
- Open VS Code and go to *File (Code if using a Mac) > Preferences > Settings*. This will open a settings dialog.
*File (Code if using a Mac) > Preferences > Settings*- Enter `insert final newline` into the search bar.
`insert final newline`- In both the *Workspace Settings* and *User Settings* tabs, ensure the checkbox labeled *Files: Insert Final Newline* is checked.
