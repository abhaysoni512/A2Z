...
> **Author:** Alex
> **Date:** Alex December 28, 2023, 4:46 pm PST January 29, 2025
...
`Random.h`- `#include "Random.h"` from any .cpp file in your project that needs to generate random numbers.
`#include "Random.h"`- Call `Random::get(min, max)` to generate a random number between `min` and `max` (inclusive). No initialization or setup is required.
