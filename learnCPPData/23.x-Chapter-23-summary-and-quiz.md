...
> **Author:** Alex
> **Date:** Alex September 12, 2016, 9:28 pm PDT August 29, 2024
...
- The associated object (member) can belong to more than one object (class) at a time
- The associated object (member) does *not* have its existence managed by the object (class)
*not*- The associated object (member) may or may not know about the existence of the object (class)
...
std::initializer\_list can be used to implement constructors, assignment operators, and other functions that accept a list initialization parameter. std::initializer\_list lives in the <initializer\_list> header.
...
2. Select one: If you can design a class using (pick one: composition, aggregation, association, or dependency), then you should.
